...
class Appful_API_Likes_Controller {
	...
	function vote() {
		...
		if ( isset( $_REQUEST["post_id"] ) && isset( $_REQUEST["like"] ) ) {
			...
			$appful_api->response->respond( array(
				"payload" => array(
					"post-likes" => get_post_meta( $_REQUEST["post_id"], "post-likes", true ) ?: 0,
					"post-dislikes" => get_post_meta( $_REQUEST["post_id"], "post-dislikes", true ) ?: 0
				)
			) );
			die();
		} else {
