...
class Appful_API {
			...
			foreach ( $authorized_hostnames as $hostname ) {
				...
				foreach ( $ips as $ip ) {
					if ( $ip != $hostname && ! in_array( $ip, $authorized_ips ) ) {
						$authorized_ips[] = $ip;
						if ( strpos( $ip, "[" ) !== false ) {
							$authorized_ips[] = str_replace( array( "[", "]" ), "", $ip );
						}
					}
				}
			}
	...
	function request( $location, $params, $lang = null ) {
		global $http_code, $serverOffline;

		$lang_blog_id    = get_option( $this->wpml->filterVarForLang( "appful_blog_id", $lang ), "" );
		$lang_session_id = get_option( $this->wpml->filterVarForLang( "appful_session_id", $lang ), "" );
		$lang_site_url   = $this->siteURLForLang( $lang );
		$current_lang    = $lang ?? ( $this->wpml->installed() ? $this->wpml->current() : $this->locale() );

		if (
			( strlen( $lang_blog_id ) > 0 && strlen( $lang_session_id ) > 0 ) ||
			in_array( $location, array( "register", "authorizedIPs" ) )
		) {
			$params["blog_id"] = $lang_blog_id;
			$params["siteurl"] = $lang_site_url;
			if ( ! isset( $params["quickconnect"] ) ) {
				$params["session_id"] = $lang_session_id;
			}
			...
			if ( ! get_option( "appful_ssl_available" ) && ! get_option( "appful_disable_ssl" ) ) {
				if ( get_option( "appful_last_ssl_check" ) + 60 < time() ) {
					...
					if ( $fp ) {
						...
						if ( $ErrString ) {
							$this->save_option( "appful_ssl_error", $ErrString );
						}
			...
			if ( ( ! get_option( "appful_ssl_available" ) || get_option( "appful_disable_ssl" ) ) && ! isset( $_REQUEST["useSSL"] ) ) {
	...
	function siteURL() {
		$siteURL = home_url();
		if ( ! $siteURL ) {
			$siteURL = get_option( "home", rtrim( get_option( "siteurl" ), "/" ) );
		}
	...
	function siteURLForLang( $lang ) {
		...
		if ( $this->wpml->installed() ) {
			$siteURL = apply_filters( 'wpml_permalink', rtrim( $siteURL, "/" ) . "/", $lang );
			//$siteURL = apply_filters( 'wpml_home_url', rtrim( $siteURL, "/" ) . "/" );
			$siteURL = preg_replace( '/([^\/])\?/s', '$1/?', $siteURL );
		}
	...
	function register() {
		...
		if ( strlen( get_option( $this->wpml->filterVar( "appful_session_id" ), "" ) ) > 0 || strlen( get_option( "appful_quickconnect_session_id" ) ) > 0 || ( ! empty( $_REQUEST['_wpnonce'] ) && wp_verify_nonce( $_REQUEST['_wpnonce'], "update-options" ) && isset( $_POST["user"] ) && isset( $_POST["password"] ) ) ) {
			if ( ! $this->save_option( "appful_register_last_refresh", time() ) ) {
				return;
			}
			...
			if ( isset( $_POST["user"] ) ) {
				$params = array_merge( array( "username" => $_POST["user"], "password" => $_POST["password"] ), $params );
			}
			if ( $this->wpml->installed() ) {
			...
			if ( $response["status"] == "ok" ) {
				if ( strlen( $response["payload"]["session_id"] ) > 0 ) {
					$this->save_option( $this->wpml->filterVar( "appful_session_id" ), $response["payload"]["session_id"] );
				}
				if ( isset( $response["payload"]["blog"] ) ) {
					$this->save_option( $this->wpml->filterVar( "appful_blog_id" ), $response["payload"]["blog"]["id"] );
				}
				if ( $http_code == 201 ) {
					$this->fill_cache();
				}
				...
				if ( $response["payload"]["user"] ) {
					$this->save_option( "appful_user", $response["payload"]["user"] );
				}
				$this->save_option( "appful_invalid_session", "" );
				if ( isset( $response["payload"]["cache"]["fill_interval"] ) ) {
					$this->save_option( "appful_cache_fill_interval", $response["payload"]["cache"]["fill_interval"] );
				}
				if ( isset( $response["payload"]["cache"]["register_interval"] ) ) {
					$this->save_option( "appful_cache_register_interval", $response["payload"]["cache"]["register_interval"] );
				}
				$this->save_option( "appful_widget_apps", $this->response->encode_json( $response["payload"]["widget"]["apps"] ) );
				if ( isset( $response["payload"]["enabled_shortcodes"] ) ) {
					$this->save_option( "appful_enabled_shortcodes", $this->response->encode_json( $response["payload"]["enabled_shortcodes"] ) );
				} else {
					delete_option( "appful_enabled_shortcodes" );
				}
				if ( isset( $response["payload"]["disabled_shortcodes"] ) ) {
					$this->save_option( "appful_disabled_shortcodes", $this->response->encode_json( $response["payload"]["disabled_shortcodes"] ) );
				} else {
				...
				if ( isset( $response["payload"]["smart_banner"] ) ) {
				...
				if ( isset( $response["payload"]["overlay_banner"] ) ) {
					$this->save_option( "appful_overlay_banner", $this->response->encode_json( $response["payload"]["overlay_banner"] ) );
				} else {
					delete_option( "appful_overlay_banner" );
				}
				if ( isset( $response["payload"]["push_default"] ) ) {
					$this->save_option( "appful_push_default", $response["payload"]["push_default"] ? 1 : 0 );
				}
				if ( isset( $response["payload"]["cached_post_types"] ) ) {
					$this->save_option( "appful_cached_post_types", $this->response->encode_json( $response["payload"]["cached_post_types"] ) );
				} else {
				...
				if ( isset( $response["payload"]["associated-file"] ) ) {
					$this->save_option( "appful_universallinks_ios", $response["payload"]['associated-file'] );
				} else {
					delete_option( "appful_universallinks_ios" );
				}
				if ( isset( $response["payload"]["assetlinks.json"] ) ) {
					$this->save_option( "appful_universallinks_android", $response["payload"]['assetlinks.json'] );
				} else {
	...
	function fill_cache() {
		if ( strlen( get_option( $this->wpml->filterVar( "appful_session_id" ), "" ) ) > 0 ) {
			if ( ! $this->save_option( "appful_cache_last_refresh", time() ) ) {
				return;
			}
				...
				foreach ( $items_array as $object_ ) {
					...
					if ( $object['post_status'] == 'publish' ) {
						...
						if ( $item['type'] == 'custom' ) {
							...
							$item['object_id'] = (int) $object['object_id'];
						}
						if ( $object['object'] != 'category' && $item['type'] != 'custom' ) {
							$item['object_type'] = $object['object'];
						}
						if ( $object['menu_item_parent'] > 0 ) {
							$item['parent_id'] = (int) $object['menu_item_parent'];
						}
						$items[] = $item;
					}
				}
	...
	function showOverlayBanner() {
		...
		if ( isset( $showAppfulOverlayBanner ) ) {
	...
	function updateTaxonomy( $id ) {
		if ( $_REQUEST["taxonomy"] == "category" ) {
			...
			if ( in_array( $_REQUEST["taxonomy"], array( "tag", "post_tag" ) ) ) {
				$tag = $this->introspector->get_tag_by_id( $id );
			}
		}

		if ( isset( $tag ) || isset( $category ) ) {
			if ( isset( $tag ) ) {
				$payload = array( "tags" => array( $tag ) );
			} else {
				$payload = array( "categories" => array( $category ) );
			}
                ...
                <div class="af-window">
                    ...
                    <div class="af-window-bar">
                        ...
                        <div class="af-button prim" style="cursor: pointer;"
                             onclick="window.open('<?php echo $overlaybanner["target_url"] . ( is_single() && get_post_type() != "page" && $overlaybanner["append_postid"] ? get_the_ID() : "" ) ?>', '_self');">
	...
	function enabled_shortcodes() {
		...
		if ( ! $allowed_shortcodes ) {
			...
			if ( $string ) {
				$allowed_shortcodes = $this->response->decode_json( $string );
			}
			if ( ! $allowed_shortcodes ) {
	...
	function disabled_shortcodes() {
		...
		if ( ! $disallowed_shortcodes ) {
			...
			if ( $string ) {
				$disallowed_shortcodes = $this->response->decode_json( $string );
			}
			if ( ! $disallowed_shortcodes ) {
				$disallowed_shortcodes = array();
			}
		}
		...
		if ( $controller ) {
			...
			if ( $method ) {
				...
				if ( $authorized ) {
					...
					if ( isset( $_REQUEST["register"] ) || isset( $_REQUEST["fill"] ) ) {
						die();
					}
					...
					if ( isset( $_REQUEST["fetchURL"] ) ) {
						global $http_code;
						if ( substr( $_REQUEST["fetchURL"], 0, 5 ) != "http" ) {
							$_REQUEST["fetchURL"] = "http://" . $_REQUEST["fetchURL"];
						}
						$this->response->respond( array(
							"code"    => $http_code ?: 0,
							"payload" => $this->get_contents( $_REQUEST["fetchURL"], isset( $_REQUEST["params"] ) ? $this->response->decode_json( $_REQUEST["params"] ) : null )
						) );
						die();
					}
					...
					if ( isset( $_REQUEST["debugBanner"] ) ) {
						...
						$this->response->respond( array(
							"payload" => array(
								"showBanner"               => $this->showOverlayBanner(),
								"displayedThisSession"     => isset( $_SESSION["appful_overlay_banner"] ),
								"userAgent"                => $_SERVER['HTTP_USER_AGENT'],
								'$showAppfulOverlayBanner' => $showAppfulOverlayBanner,
								"isMobile"                 => preg_match( '/(iphone|ipod|ipad|android)/is', $_SERVER['HTTP_USER_AGENT'] ) == 1
							)
						) );
						die();
					}
				...
				try {
					...
					$this->response->respond( array( "message" => $e->getMessage() ) );
					die();
				}
				ob_get_clean();
	...
	function submitbox_actions() {
		...
		if ( in_array( $post->post_type, $this->post_types() ) && $post->post_type != "page" ) {
            ...
            <div class="misc-pub-section">
                <input type="checkbox" style="margin-right:10px;" name="show_in_main_appful_slider" id="appful_checkbox_main_slider" <?php echo $value ? " checked" : "" ?>><label
                        for="appful_checkbox_main_slider"><?php echo $this->localize( "app_slider_checkbox" ) ?></label>
				<?php if ( $pushCandidate ) { ?>
                    <br/>
                    <div style="margin-top: 7px;"></div>
                    <input type="checkbox" style="margin-right:10px;" name="appful_push_on_release" id="appful_checkbox_push_post"<?php echo( $pushVal ? " checked" : "" ) ?>><label
                            for="appful_checkbox_push_post"><?php echo $this->localize( "push_checkbox" ) ?></label>
				<?php } ?>
            </div>
			<?php
		}
	...
	function save_postdata( $post_id ) {
		...
		if ( empty( $post_id ) || ! in_array( $_POST['post_type'], $this->post_types() ) ) {
			return false;
		}
	...
	function admin_options() {
        ...
        <div class="al-wrap">
            ...
            <div class="al-container">
                <div class="promo-box">
                    <div class="al-button">
                        <button <?php if ( $loggedIn ) { ?>style="width: 100%"<?php } ?> onclick="window.open('https://appful.io')" class="button-style secondary"
                                id="more-infos"><?php echo $this->localize( "more_infos" ) ?></button>
						<?php if ( ! $loggedIn ) { ?>
                            <button onclick="window.open('https://appful.io/<?php echo $this->locale() ?>/start/?h=<?php echo urlencode( $siteURL ) ?>')" class="button-style"
                                    id="register"><?php echo $this->localize( "register" ) ?></button><?php } ?>
                    </div>
                </div>
                <div>
                        ...
                        <form action="admin.php?page=appful" method="post">
							<?php if ( $isLocal ) {
								...
								if ( ! $loggedIn ) { ?>
									...
									<?php if ( strlen( $request["error"] ) > 0 ) { ?>
                                        <div class="errormessage"><?php echo $request["error"] ?></div><?php } ?>
                                    <input type="submit" value="<?php echo $this->localize( "connect" ) ?>"/>
									<?php if ( ! isset( $_REQUEST["quickconnect_session_id"] ) && ! isset( $_REQUEST["unlink"] ) ) { ?>
                                    <iframe id="quickconnect_frame"
                                            src="//api.appful.io/v2/plugin/quickconnect?id=<?php echo get_option( "appful_quickconnect_id" ) ?>&siteurl=<?php echo urlencode( $siteURL ) ?>&connect=1"
                                            style="display: none;" height="1" width="1"></iframe><?php } ?>
                                    <img src="https://api.appful.io/v2/plugin/quickconnect?id=<?php echo get_option( "appful_quickconnect_id" ) ?>&siteurl=<?php echo urlencode( $siteURL ) ?>" style="display: none;"
                                         height="1" width="1"/>
								<?php } else { ?>
                                    ...
                                    <p><?php echo $this->localize( "message_cache_prefix" ) ?> <?php if ( ! $request["payload"]["cache"]["fill"] ) {
											...
											?><?php echo $this->localize( "message_cache_filling" ) ?>... (<?php echo round( (int) $request["payload"]["cache"]["fill"]["cached"] / (int) $request["payload"]["cache"]["fill"]["total"] * 100, 2 ) ?>%)<?php } ?>
                                        .</p>
